...
<?php 
	...
	//weights
	$sql = "SELECT wk,weight FROM weights;";
	if (!$db->query($sql)) {
  		printf("Errorcode: %d\n", $db->errno);
  		exit;
	} else {
	...
	foreach($teams as $k=>$team) {
		...
		$sql = "UPDATE wei_ratings SET pyth=" . $team->pyth() . ", pace=" . $team->num('pace',4) . 
	...
	$db->close();
	if ($i == 351) {echo "All weighted ranking records updated.";} else {$i . "  weighted ranking records updated.";}
